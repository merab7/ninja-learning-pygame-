...
class Game:
    ...
    def run(self):
        while True:
            ...
            #self.scroll[0] += 1 #camera moving (but i want camera to be focused on the player)
            
            self.scroll[0] += (self.player.rect().centerx - self.display.get_width()/2 - self.scroll[0]) / 30
            self.scroll[1] += (self.player.rect().centery - self.display.get_width()/2 - self.scroll[1]) / 30
            render_scroll = (int(self.scroll[0]), int(self.scroll[1]))#resolving this inconsistent moving of camera doe to floaty character of scroll
